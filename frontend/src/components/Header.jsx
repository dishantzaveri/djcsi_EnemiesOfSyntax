...
export default function Header({ photo, email }) {
  return (
    <header className='flex flex-col xs:flex-row justify-between items-center w-full mt-3 border-b pb-7 sm:px-4 px-2 border-gray-500 gap-2'>
      <Link to='/dream' className='flex space-x-2'>
        <img alt='header text' src='/bed.svg' className='sm:w-10 sm:h-10 w-9 h-9' width={24} height={24} />
        <h1 className='sm:text-3xl text-xl font-bold ml-2 tracking-tight'>roomGPT.io</h1>
      </Link>
      {email ? (
        <div className='flex items-center space-x-4'>
          <Link to='/dashboard' className='border-r border-gray-300 pr-4 flex space-x-2 hover:text-blue-400 transition'>
            <div>Dashboard</div>
          </Link>
          <Link
            to='/buy-credits'
          ...
          {photo ? (
            <img alt='Profile picture' src={photo} className='w-10 rounded-full' width={32} height={28} />
          ) : (
