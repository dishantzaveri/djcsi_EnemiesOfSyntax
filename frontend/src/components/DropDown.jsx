...
export default function DropDown({ theme, setTheme, themes }) {
  return (
    <Menu as="div" className="relative block text-left">
      ...
      >
        <Menu.Items
          className="absolute h-80 left-0 z-10 mt-2 w-full origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none overflow-auto"
          key={theme}
        >
          <div className="">
            {themes.map((themeItem) => (
              <Menu.Item key={themeItem}>
                {({ active }) => (
                  ...
                  >
                    <span>{themeItem}</span>
                    {themeItem === theme ? <CheckIcon className="w-4 h-4 text-bold" /> : null}
                  </button>
                )}
              </Menu.Item>
            ))}
